...
- Online preview / produkční doména: https://www.bemer-lucie.cz/
...
- Každá designová změna se vztahuje k produkční doméně `https://www.bemer-lucie.cz/`. Při ověřování, kontrole výsledku a komunikaci o webu se pracuje s touto adresou jako s cílovou stránkou.
...
Použij tento projektový kontext jako paměť práce na webu `Lucka stránky`. Jedná se o statický microsite web pro BEMER terapii v `C:\Users\malp\Documents\Lucka stránky`, s produkční doménou `https://www.bemer-lucie.cz/`. Zachovej aktuální směr: klidný, důvěryhodný, teplý design, bez full-width hero fotografie, protože předchozí pokusy s fotografickým hero backgroundem byly vrácené kvůli viditelnému předělu. Při každé designové změně pracuj s produkční doménou jako cílovou stránkou a bez výjimky zvaž dopad na CMS. Pokud změna ovlivňuje obsah, viditelnost, strukturu, obrázky, odkazy, seznamy nebo cokoli spravovatelného v administraci, automaticky aktualizuj `content/site.json`, `admin/config.yml` a odpovídající napojení v `index.html` / `script.js`. CSS používej pouze pro prezentaci, ne jako náhradu CMS stavu. Respektuj aktuální layout a ukládej změny do git historie.
...
- 2026-05-12: Upřesnění: do hero byla vrácena úplně první orbitální background grafika z varianty 2, tedy pravostranný abstrakt s kruhovými linkami, oranžovým středem a body. Spodní “podložkové” vlnění z pozdějších experimentů bylo odstraněno, full-width layout hero zůstal zachován.
- 2026-05-12: Pro hero orbitální grafiku byl přidán jemný parallax efekt. Dekorativní `.hero-art` se při scrollu posouvá pomaleji vůči obsahu, takže působí jako pozadí držené na místě, zůstává však oříznuté uvnitř hero sekce a respektuje `prefers-reduced-motion`.
- 2026-05-12: Hero background byl vytažen nahoru pod sticky menu, aby mezi navigací a hero nevznikala světlá mezera. `.page-shell` už nemá horní padding a hero používá záporný horní margin s odpovídajícím paddingem, takže orbitální background začíná vizuálně hned za menu a parallax působí od začátku stránky.
- 2026-05-12: Mobilní mikrocirkulační sekce byla upravena: insight box nad videem se na mobilu skrývá a video má rozšířené potlačení nativních mobilních ovládacích prvků/časového overlaye, včetně JS vynucení muted inline autoplay bez controls.
...
- 2026-05-16: Na větvi `codex/decap-cms-content` bylo připraveno jednoduché CMS řešení pro GitHub Pages pomocí Decap CMS bez nového buildu a bez změny layoutu. Přibyla administrace v `admin/`, obsahový soubor `content/site.json`, dynamické naplnění textů a vybraných obrázků ze skriptu, opakovatelný seznam referencí s náhodným pořadím a dynamicky generovanými tečkami carouselu, fallback na původní HTML při chybě JSONu a bezpečnější CSS pro delší texty v kartách, FAQ, referencích a tlačítkách. Soubory: `index.html`, `script.js`, `styles.css`, `content/site.json`, `admin/index.html`, `admin/config.yml`, `PROJECT_CONTEXT.md`.
- 2026-05-16: Do CMS bylo doplněno pevné pole `Značka / Logo`, které mění logo současně v hlavičce i patičce. Logo má alternativní text, zachovává stávající CSS velikosti a při chybě načtení padá zpět na původní `logo.png`.
...
- 2026-05-16: Do projektového kontextu byla doplněna závazná pravidla práce: každá designová změna se vztahuje k produkční doméně `https://www.bemer-lucie.cz/` a vždy se musí bez výjimky posoudit dopad na CMS. Pokud dopad existuje, musí se automaticky aktualizovat `content/site.json`, `admin/config.yml` a napojení v `index.html` / `script.js`; CSS nesmí nahrazovat CMS stav.
- 2026-05-16: Spodní karty v sekci mikrocirkulace byly doplněny o CMS pole `Aktivní`. Front-end nyní renderuje pouze aktivní karty, nastavuje `data-count` podle jejich počtu a CSS podle toho mění počet sloupců. Při jedné kartě je jeden sloupec, při dvou dvě stejně široké karty, při třech tři karty na desktopu a responzivní zalomení na menších šířkách.
- 2026-05-16: Sekční nadštítky (`eyebrow`) byly v CMS převedeny na objekt s poli `Aktivní` a `Text`. Front-end používá `data-cms-active` a `data-cms-text` pro každý nadštítek, takže při vypnutí v CMS prvek zmizí z toku stránky a nezabírá vertikální prostor.
- 2026-05-16: Karty v sekci `Kde může být BEMER vhodnou podporou` dostaly v CMS volitelné pole `Ikona` (`image.src`, `image.alt`). Lze nahrát PNG s průhledností nebo SVG; pokud je pole prázdné, front-end ponechá původní výchozí kruhovou ikonku.
- 2026-05-16: Karty v sekci `Jak spolupráce probíhá` jsou v CMS ponechané jako přidávatelný/odebíratelný seznam. Front-end podle počtu položek nastavuje `data-count` a CSS upraví rozložení: na desktopu 1-4 sloupce podle počtu, nad 4 položky se zalamují do další řady, na tabletu max 2 sloupce a na mobilu 1 sloupec.
- 2026-06-13: Do CMS byly doplněny přepínače viditelnosti pro úvodní texty sekcí `S čím může BEMER pomoci`, `Jak spolupráce probíhá`, `Reference` a pro CTA box pod kroky spolupráce. Data jsou uložená jako objekty s poli `active` a `text` (CTA navíc `label`), front-end používá `data-cms-active` a skryté bloky nezabírají vertikální místo. Soubory: `content/site.json`, `admin/config.yml`, `index.html`, `styles.css`, `PROJECT_CONTEXT.md`.
- 2026-06-13: Karty v sekci `Jak spolupráce probíhá` dostaly v CMS volitelné pole `Ikona místo čísla` (`steps[].image.src`, `steps[].image.alt`). Pokud je ikonka nahraná, zobrazí se místo oranžového čísla; pokud chybí, zůstává původní číslo kroku jako fallback. Soubory: `content/site.json`, `admin/config.yml`, `index.html`, `script.js`, `styles.css`, `PROJECT_CONTEXT.md`.
- 2026-06-13: Nadpis mikrocirkulační sekce dostal samostatnou typografickou šířku, aby se aktuální CMS text `Mikrocirkulace jako základ optimálního fungování organismu` na desktopu i mobilu lámal nejvýš na 3 řádky. Jde jen o designovou úpravu CSS, bez změny CMS dat. Soubory: `index.html`, `styles.css`, `PROJECT_CONTEXT.md`.
- 2026-06-13: Karty v sekci `Jak spolupráce probíhá` dostaly v CMS volitelný blok `Cena / zvýrazněný blok` (`steps[].price.active`, `steps[].price.bold`, `steps[].price.text`). Blok se renderuje odděleně pod hlavním textem s horním odsazením a linkou; lze ho vypnout nebo celý zobrazit tučně. U karty `Pronájem` je aktivní a obsahuje ceny, u karty `Prodej` je připravený, ale neaktivní. Soubory: `content/site.json`, `admin/config.yml`, `index.html`, `script.js`, `styles.css`, `PROJECT_CONTEXT.md`.
- 2026-06-13: Reference dostaly v CMS globální přepínače `Zobrazovat štítek reference` a `Zobrazovat krátký popisek pod jménem` (`testimonials.display.showTags`, `testimonials.display.showContexts`) a zároveň přepínače u každé položky (`items[].tagActive`, `items[].contextActive`). Obě pole jsou aktuálně vypnutá globálně i u jednotlivých referencí. Záložní HTML reference byly zbavené štítků a popisků a cache query u `script.js` byla navýšená, aby produkce nenačítala staré vykreslení. Soubory: `content/site.json`, `admin/config.yml`, `index.html`, `script.js`, `PROJECT_CONTEXT.md`.
- 2026-06-13: Karty v sekci `Co vám mohu nabídnout` dostaly plnobarevnou oranžovou variantu inspirovanou referencí: sytý oranžový gradient, světlý text, jemné teplé světlo a výraznější kruh čísla. Změna přenáší pouze barevnost, layout a obsah karet zůstaly stejné. Cache query u `styles.css` byla navýšená. Soubory: `index.html`, `styles.css`, `PROJECT_CONTEXT.md`.
- 2026-06-15: V plnobarevných kartách sekce `Co vám mohu nabídnout` byl nadpis přesunut do horní řádky vlevo od číselné ikonky a zvětšen. Karta používá grid, text i cenový blok dál běží přes celou šířku a mobilní layout už nepoužívá absolutně umístěné číslo. Cache query u `styles.css` byla navýšená. Soubory: `index.html`, `styles.css`, `PROJECT_CONTEXT.md`.
- 2026-06-15: Horní řádka plnobarevných karet byla otočena podle zadání: číselná ikonka je znovu vlevo a nadpis je napravo od ní. Nadpis je lehce menší než v předchozí variantě, aby působil vyváženě vedle kruhu. Cache query u `styles.css` byla navýšená. Soubory: `index.html`, `styles.css`, `PROJECT_CONTEXT.md`.
